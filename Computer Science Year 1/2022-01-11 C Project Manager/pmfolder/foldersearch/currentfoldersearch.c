...
int list_file_type_current(char type, char result[MAX_NUM][NAME_LENGTH])
{
...
int currentfoldersearch(void)
{
  ...
  int num_found;                                             
  printf("Directories\n");                                  
  num_found= list_file_type_current(typ, result_list);                       //call our function, asking for typ (_DIR) and telling it where to put the results
  for (int i=0;i<num_found;i++)
    printf("%s\n",result_list[i]);

  printf("\nFiles\n");
  typ=_FILE;
  num_found= list_file_type_current(typ, result_list);                       //call our function, asking for typ (_FILE) and telling it where to put the results.  Is overwriting the results array a good idea?
  for (int i=0;i<num_found;i++)
    printf("%s\n",result_list[i]);
  return 0;
}
